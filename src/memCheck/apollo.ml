...
let apollo_func (f: fundec) (cfile: file) (fsm: bool): unit = 
    ...
    List.iter 
      (fun s -> 
         try
           ...
           let safe_return = 
             if fsm then ( 
               verify_state_with_post_fsm f.svar.vname (return_eop, formals) return s
             ) else (
               verify_state_with_post f.svar.vname (return_eop, formals) return s
             )
           in
...
let apollo_func_simple (f: fundec) (cfile: file): unit =
  apollo_func f cfile true
;;


let apollo_func_fsm (f: fundec) (cfile: file): unit =
  apollo_func f cfile false
;;
