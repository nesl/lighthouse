...
\code{full} or \code{empty} memory buffer.
%
...
%
An expression denoted as \code{full} in the preconditon describes memory that
...
Figure~\ref{fig:spec}.
%
The postcondition for \code{ker\_malloc} notes that caller of
\code{ker\_malloc} must take ownership of the returned data.
%
The \code{ker\_free} function specifies that the first formal parameter points
