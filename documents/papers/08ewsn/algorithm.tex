...
valid in \code{outgoing} specifications)
%
\item Global variable identified by variable name
%
\end{itemize}
%
The specification uses \code{incoming} and \code{outgoing} to describe memory
that is being released into and out from the function, respectively.
%
An \code{outgoing} expression qualified with \code{release} describes dynamic
...
%
An \code{incoming} expression qualified with \code{release} describes dynamic
...
Figure~\ref{fig:spec}.
%
The \code{release} qualifier on the \code{outgoing} return statement notes
that the caller of \code{ker\_malloc} must take ownership of the returned
data.
%
The \code{ker\_free} function specifies that the \code{incoming} first formal
...
\code{incoming} parameters denoted as \code{release}, and the function's exit
nodes are considered release points for \code{outgoing} expressions denoted as
\code{release}.
