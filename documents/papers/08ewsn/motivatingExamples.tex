...
to send a \code{TOS\_Msg} over the network.
%
...
%
The \code{SendMsg} interface contract augments the {\tt TOS\_Msg} structure
with a {\tt msg\_state} field to describe the owner of the message as {\tt
USER\_OWNED} or {\tt OS\_OWNED}.
%
A component sends a \code{USER\_OWNED} message using \code{send} and the
message state is updated to be \code{OS\_OWNED}.
%
When the underlying system returns the message in a \code{sendDone} event,
the state is returned to \code{USER\_OWNED}.
%
The runtime system flags an error should a component succeed in sending an
\code{OS\_OWNED} message. 
