...
of memory.  \mynote{I wanted to explicitly list the functions we
  count.  Did I get them all?}
...
source files, of which 35 generated warnings.  \mynote{Not broken down
  into number of modules, like the previous study?}  A detailed breakdown of
...
were introduced into the code. \mynote{Was the loader part of the
  experiments described above, or is this a separate thing?  If so,
  say so.  Is this one of the conditional memory leaks described for
  the SOS kernel?  Doesn't seem like it exactly.}
...
reads, ``Fixed another memory leak.''  \mynote{Presumably now
  Lighthouse doesn't signal a warning any more?  If so, please say
  this.}
